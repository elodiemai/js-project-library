...
document.getElementById('cancel').addEventListener('click', displayForm);
document.getElementById('checkbox-available').addEventListener('change', displayDueDateField);
...
function updateTable() {
    ...
    const table = document.querySelector('table');
    table.appendChild(document.createElement('tbody'));
    myLibrary.map((book, i) => addToTable(book, i));
}


function addToTable(book, index) {
    ...
    markAvailableBtn.addEventListener('click', () => markAvailable(index));
...
function removeFromTable(index) {
    const tbody = document.querySelector('tbody');
    const rowToRemove = document.querySelector(`tr[data-attribute="${index}"]`);
